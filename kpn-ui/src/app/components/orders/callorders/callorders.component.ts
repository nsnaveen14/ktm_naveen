...
@Component({
  selector: 'app-callorders',
  imports: [MatTableModule,MatPaginator, MatPaginatorModule,MatFormFieldModule, MatInputModule,MatCheckboxModule],
  templateUrl: './callorders.component.html',
  styleUrl: './callorders.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CallordersComponent implements OnInit{
  @ViewChild(MatPaginator) paginator: MatPaginator|any;

  @Input() callTableDataSource =new MatTableDataSource<InstrumentMiniTableSource>();
  callTableColumns: string[] = ['select','strike','instrument_token','tradingsymbol','last_price'];
  ...
  ngAfterViewInit() {
    this.callTableDataSource.paginator = this.paginator;
  }

  applyFilter(event: Event) {
      const filterValue = (event.target as HTMLInputElement).value;
      this.callTableDataSource.filter = filterValue.trim().toUpperCase();
     ...
     isAllSelected() {
      ...
      const numRows = this.callTableDataSource.data.length;
     ...
     toggleAllRows() {
      ...
      this.selection.select(...this.callTableDataSource.data);
    }
